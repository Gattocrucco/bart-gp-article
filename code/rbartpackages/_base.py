...
class RObjectABC(abc.ABC):
    ...
    def __init__(self, *args, timeout=None, retries=0, **kw):
        ...
        robjects.r(f'library({library})')
            # TODO I would like to do loadNamespace('<library>') and then always
            # use <library>::<thing>. However this does not work with methods
            # (see __init_subclass__). I have to look up how to reference
            # methods directly in a namespace. Alternatively, I could do
            # library(<library>, quietly=TRUE), but I prefer not to suppress
            # eventual errors.
    ...
    def __init_subclass__(cls, **kw):
        ...
        add_r_help_to_obj(cls, library, name)

def add_r_help_to_obj(obj, library, name):
    package_help = robjects.help.Package(library)
    page = package_help.fetch(name)
    if obj.__doc__ is None:
        obj.__doc__ = ''
    obj.__doc__ += 'R documentation:\n' + page.to_docstring()

def make_r_function(library, name):
    
    def func(*args, **kw):
        robjects.r(f'library({library})')
        func = robjects.r(name)
        rout = func(*RObjectABC._args2r(args), **RObjectABC._kw2r(kw))
        pyout = RObjectABC._r2py(rout)
        return pyout

    add_r_help_to_obj(func, library, name)
    pyname = name.replace('.', '_')
    func.__name__ = pyname
    func.__qualname__ = f'{library}.{pyname}'
